...
from enumeration.manager import EnumerationError
from enumeration.manager import get_next_position
...
class GetNextTestCase(TestCase):
    ...
    def test_monthly_reset(self):
        ...
        c = Counter.objects.create(
            sequence=s, period=datetime.date(2012, 1, 1), position=5
        )
        self.assertEqual(get_next_position(s, d), 6, "Counter position")
        ...
        c = Counter.objects.create(
            sequence=s, period=datetime.date(2012, 2, 1), position=5
        )
    ...
    def test_yearly_reset(self):
        ...
        c = Counter.objects.create(
            sequence=s, period=datetime.date(2013, 1, 1), position=5
        )
